...
namespace
{
    ...
    // channel 0 of the output, sample for sample.
    std::vector<float> runThroughDSP (const std::function<float (int)>& generator, int numSamples,
                                       RunConfig cfg, bool& hadNonFinite)
    {
        AirBandDSP dsp;
        dsp.prepare (kSampleRate, kBlockSize, 2);
        dsp.setParameters (cfg.midAirDb, cfg.highAirDb, cfg.blend, cfg.outputDb, cfg.deEss, cfg.comp);
...
int main()
{
    ...
    // must actually pass audio through unchanged -- not silence it, not
    // corrupt it. This is the direct check against "tests pass, output is
    // dead". ---
    {
        std::printf ("Test 1: transparent settings pass audio through unchanged\n");
        auto gen = sineWave (1000.0, 0.25f);
        bool nonFinite = false;
        auto out = runThroughDSP (gen, twoSeconds, RunConfig {}, nonFinite);

        double maxAbsDiff = 0.0;
        for (int i = 0; i < twoSeconds; ++i)
            maxAbsDiff = juce::jmax (maxAbsDiff, (double) std::abs (out[(size_t) i] - gen (i)));

        check (! nonFinite, "output contains no NaN/Inf");
        check (maxAbsDiff < 1.0e-5, "output is bit-identical to input (max diff " + std::to_string (maxAbsDiff) + ")");
    }
